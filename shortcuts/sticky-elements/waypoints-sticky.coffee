...
# - stuckClass: The class that is added to the sticky element when the waypoint
#   is reached. Users should style this class to add fixed positioning and
#   whatever other styles are necessary for their particular design.
...
# is part of the normal document flow until it reaches the top of the viewport,
# where it gains a fixed position state.
...
# This shortcut does take care of the most common pitfall in previous versions
# of Waypoints: Using the sticky element as the waypoint. Fixed position
# elements do not work well as waypoints since their position in the document
# is constantly changing as the user scrolls (and their position relative to
# the viewport never does, which is the whole point of Waypoints).
# This shortcut will create a wrapper element around the sticky element that
# acts as the actual waypoint, as well as a placeholder for the waypoint in
# the document flow, as fixed positioning takes an element out of flow and
# would otherwise effect the page layout. Users are recommended to define any
# margins on their sticky elements as margins on this wrapper instead.
